...
class MobileCoreService(BaseService):
    ...
    def _emit_pdu_session(self) -> None:
        ...
        market = random.choice(["NYC", "DFW", "SEA", "ATL", "CHI"])
        dnn = random.choice(["internet", "ims", "emergency"])
        self.emit_log(
            "INFO",
            f"[5G] pdu_session_create imsi=310{random.randint(10,99)}{random.randint(100000000,999999999)} "
            f"dnn={dnn} market={market} status=ESTABLISHED latency_ms={random.randint(20,120)}",
            {
